...
public class TempFolderHandler implements Closeable {
	...
	public TempFolderHandler(Function<Path, Optional<? extends IOException>> output) throws IOException {
		this.toggle = true;
		this.output = output;
		this.t1 = createTempFolder();
		this.t2 = createTempFolder();
	}
	...
	private static Path createTempFolder() throws IOException {
		...
		int i = 0;
		do {
			ret = parent.resolve(Long.toHexString(System.currentTimeMillis())+"-"+Integer.toHexString(RND.nextInt()));
			i++;
			if (i>=1000) {
	...
	public void reset() throws IOException {
		...
		if (!isEmpty(getOutput())) {
			...
			DefaultTempFileWriter.deleteRecursive(getOutput(), false);
		} else {
	...
	public void close() throws IOException {
		...
		try {
			...
			DefaultTempFileWriter.deleteRecursive(t1);
			DefaultTempFileWriter.deleteRecursive(t2);
			t1 = null;
			t2 = null;
		}
	}
